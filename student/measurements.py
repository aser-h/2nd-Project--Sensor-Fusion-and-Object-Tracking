...
class Sensor:
    ...
    def in_fov(self, x):
        ...
        pos_veh = np.ones((4, 1)) 
        pos_veh[0:3] = x[0:3] 
        pos_sens = self.veh_to_sens*pos_veh #  vehicle into lidar coordinates mtx
...
#         if self.name == 'lidar':    # By removing this line
...
class Measurement:
    ...
    def __init__(self, num_frame, z, sensor):
        ...
        if sensor.name == 'lidar':
            ...
            self.width = z[2]
            self.length = z[3]
            self.R = np.matrix([[pow(sigma_cam_i,2), 0],  # measurement noise covariance matrix
                                [0, pow(sigma_cam_j,2)]])
